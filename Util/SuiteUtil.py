...
# 读取重复次数
cf = configRead.configFileAction()
replay = cf.get("util","replay")
            ...
            while success_flag:
                if _isnotsuite(test):
                    ...
                    if (getattr(test.__class__, '_classSetupFailed', False) or
                        ...
                        if class_num > failcount:
                            ...
                            log.warning('类%s第%s次重新初始化执行' % (test.__class__, class_num))
                ...
                if result.result[-1][0] == 1 or result.result[-1][0] == 2:  # 结果为fail和err用例判断
                    if case_num > failcount:
                        ...
                        log.warning('用例%s第%s次重新执行' % (test, case_num))
